...
class CallLog {
  ...
  static Future<List<CallLogEntry>> get() async {
    ...
    final List<CallLogEntry> entries =
        result?.map((dynamic m) => CallLogEntry.fromMap(m)).toList() ??
            <CallLogEntry>[];
    return entries;
  }
  ...
  static Future<Iterable<CallLogEntry>> query({
    ...
    final Iterable<dynamic>? records =
        await _channel.invokeMethod('query', params);
    final List<CallLogEntry> entries =
        records?.map((dynamic m) => CallLogEntry.fromMap(m)).toList() ??
            <CallLogEntry>[];
    return entries;
  }
}
...
class CallLogEntry {
  ...
  Map<String, dynamic> toJson() {
    int unixTimestamp =
        timestamp ?? 0; // Replace with your Unix timestamp in milliseconds
    ...
    return {
      ...
      'duration': duration,
      'timestamp': DateFormat('dd-MM-yyyy HH:mm:ss').format(dateTime),
